...
class CQLSAC(nn.Module):
    ...
    def __init__(self,
        ...
        # CQL params
        self.with_lagrange = with_lagrange #False
        #self.temp = temp
    ...
    def calc_policy_loss(self, states, alpha, bc=True):
        ...
        if bc:
            ...
            action = dist.sample()
            ventstatus = states[:, -2] * 2
            extustatus = states[:, -1]
            status = F.one_hot(torch.add(ventstatus.to(torch.int64), extustatus.to(torch.int64)))
            action = F.one_hot(action)
            bc_loss = F.mse_loss(status.to(torch.float32), action.to(torch.float32))
            actor_loss += bc_loss
